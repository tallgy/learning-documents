...
https://juejin.cn/post/6844904079353708557		很面试官方
https://juejin.cn/post/6961349015346610184		渲染引擎和js引擎的关系
```
...
<img src="images/image-20210919100848921.png" alt="image-20210919100848921" style="zoom: 50%;" />
...
```
https://www.cnblogs.com/dennisj/p/12550996.html	setTimeout和setImmediate到底谁先执行，本文让你彻底理解Event Loop
https://juejin.cn/post/6844903814508773383	宏任务和微任务
```
...
<img src="images/image-20210918164156396.png" alt="image-20210918164156396" style="zoom:67%;" />

```
setImmediate setTimeout的执行问题
	对于这两个有点难，先放下。。但是都是属于宏任务的，只是因为里面的事件循环的timers，poll和check阶段的时候的一些问题，对于这几个阶段没有了解
```

<img src="images/image-20210918163713806.png" alt="image-20210918163713806" style="zoom:50%;" />
...
<img src="images/image-20210918163613154.png" alt="image-20210918163613154" style="zoom:67%;" />
...
常见微任务
  Promise
  process.nextTick(Node.js)		这里nexttick会先于微任务执行
  Object.observe
  MutaionObserver
```
...
一次事件循环被称为tick或者loop
宏任务队列的第一个会在下一个tick里面执行
而微任务会在当前tick执行之后执行（作业队列，类似于存在于tick的尾部）
	微任务产生的微任务也会在本次tick执行，而宏任务放于宏任务的队尾
```
...
个人说法，就是会把异步的操作分为两种，一种是 settime和setinterval之类的
另一种是promise的then方法，这里，promise的链式调用里面，第一个then方法没有执行是不会将第二个then方法放入微任务的。
```
...
通过事件循环来配合了渲染引擎和js引擎 event loop
...
event loop 加上“急事”处理的快速通道，这就是微任务 micro tasks。

**requestAnimationFrame 是每次 loop 结束发现需要渲染，在渲染之前执行的一个回调函数，不是宏微任务。**

fiber 化，	requestIdleCallback 会在每次 check 结束发现距离下一帧的刷新还有时间，就执行一下这个。如果时间不够，就下一帧再说
...
https://juejin.cn/post/6844903505983963143	这个可以
```
...
```
https://blog.csdn.net/weixin_38418878/article/details/98487188	数字签名、数字证书与CA
https://www.jianshu.com/p/29e0ba31fb8d	一文看懂HTTPS、证书机构（CA）、证书、数字签名、私钥、公钥
https://www.cnblogs.com/liyuhui-Z/p/7844880.html	https建立连接的详细过程
```
...
```
使用非对称加密算法解决密钥交换问题
然后产生对称算法，将后面要使用的会话密钥，通过上面传输来的密钥进行加密，
然后对方得到密文后解密，于是就完成了对称密钥的交换，后续就使用对称算法完成数据交换
...
数字签名
	发送方会把要传输的数据通过hash算出来，这个就是信息摘要。这个摘要还要进行私钥加密
		因此，只要传输的数据我们接收方进行hash后，然后判断是否和信息摘要相同就可以知道信息是否正确
		
如何验证数字签名
	接收方会先用公钥解密，获得hash值1
	然后使用hash算法算出明文的hash值，为hash2，
	只要hash1和hash2相等，则表示内容一致
	
这里的问题，
	第一个，既然都hash了，为什么还要私钥加密，
		因为，hash只是保证了别人不会知道内容，但是可以直接将证书内容进行全部伪造，不需要知道证书的内容是什么，所以，这个时候公私钥的作用就有了。
	第二个，如果公钥被替换了怎么办。这个时候，数字证书的作用就诞生了。
```

<img src="images/image-20210915202457356.png" alt="image-20210915202457356" style="zoom:67%;" />
...
发送客户端的加密协议以及版本，ssl和tls
服务端旋转合适的加密协议，并返回证书，含有公钥
客户端验证证书合法性
客户端生成对称密钥，通过公钥加密，发送到服务器
服务器使用私钥解密，获取对称密钥
后续就使用了对称密钥进行加密和解密了
```

![image-20210915203550508](images/image-20210915203550508.png)
...
## 在script标签的async和defer

```
https://www.cnblogs.com/jiasm/p/7683930.html
```

<img src="images/image-20210916092750564.png" alt="image-20210916092750564" style="zoom:50%;" />

**普通script**

<img src="images/image-20210916092729861.png" alt="image-20210916092729861" style="zoom:67%;" />
...
<img src="images/image-20210916092809993.png" alt="image-20210916092809993" style="zoom:67%;" />

**async**

`async`脚本的加载不计入`DOMContentLoaded`事件统计，也就是说下图两种情况都是有可能发生的

<img src="images/image-20210916092832238.png" alt="image-20210916092832238" style="zoom:67%;" />

<img src="images/image-20210916092903081.png" alt="image-20210916092903081" style="zoom:50%;" />
...
```
https://blog.csdn.net/qq_42002487/article/details/115608197
```
...
简单来说，纯函数就是 没有依靠外界变量，不会修改外界变量，就只是通过参数进行传递变量，因此不会因为外界变量的改变而改变
```

## 了解class写法吗？

```
https://blog.csdn.net/weixin_44691513/article/details/108416033
```
...
```
https://www.cnblogs.com/devinzh/p/14864040.html
```

## css包含块

```
https://www.cnblogs.com/liuhao-web/p/7404619.html
```
...
## 画了一颗树 说出树的前序 中序 后序遍历

<img src="images/image-20210918101419864.png" alt="image-20210918101419864" style="zoom:33%;" />
...
<img src="images/image-20210918110553252.png" alt="image-20210918110553252" style="zoom:33%;" />
...
<img src="images/image-20210918110918660.png" alt="image-20210918110918660" style="zoom:50%;" />

## 给两个数组，如何判断两个数组是否有相同元素 讲思路和代码的时空复杂度 

```
使用hashmap或者对象进行存储，数组的值为键，如果出现一个数是存在于键的，说明了这个数以前出现过，所以相同。
```
...
## 给一个数组 如何让A+B=C 使C的值最大 将思路 说时空复杂度 （我的思路不是最优让我讲了如何优化）

```
先排序，然后从后面向前逐步相加，每次相加进行判断
```
...
```
cnblogs.com/lichunyang321/p/9594196.html
```
...
## 原生js了解多少？能不能讲一下事件委托？事件委托会存在什么问题？（实在想不到）所以你觉得事件委托没有问题是吗？ 

```
https://www.cnblogs.com/dhsz/p/8883645.html	事件委托
```

```
事件委托，简单来说就是，利用了事件的冒泡的原理，将一些相同的或向似的事件合并放到了父元素上面，通过target的方式再确认是点击的哪个元素，以此来进行操作。
```

##  你知道为啥js是脚本语言吗？脚本语言有啥特点？你知道编译型语言和解释型语言的区别吗？为啥要分编译型语言和解释性语言？js可以编译吗？ 

```
因为 脚本语言的作用是为了缩短编译和执行之间的过程，而js是作用于浏览器上面的，所以，为了有一个更快的执行速度，和更好的用户体验，所以js就被作为了脚本语言。
```
...
<img src="images/image-20210918194838236.png" alt="image-20210918194838236" style="zoom:67%;" />
...
<img src="images/image-20210918202002220.png" alt="image-20210918202002220" style="zoom:67%;" />
...
响应式分为，observer， dep 和 watcher
observer 是观察对象属性的变化，
dep 是observer收集和通知订阅者，
watcher 就是订阅者，它存在于dep属性的subs数组属性内，负责在数据变化时执行操作。
```

## 什么情况下会触发diff

```
https://blog.csdn.net/weixin_40599109/article/details/107645834		diff算法执行过程
```

```
patch函数，比较新旧结点，
diff会调用patch函数，通过传递一个新的node和旧的node值来进行判断是否有修改，然后再判断是否要把新的传递给旧的
```

## diff算法的时间复杂度

```
https://juejin.cn/post/6969860972869582861	Vue2 VS Vue3 Diff算法的比较
```
...
```
https://blog.csdn.net/u013243347/article/details/80051944
```

使用的矩阵matrix，也就是transform模板函数
...
其次使用
后面的prototype和
前面的__proto__进行判断，没有就进行迭代，知道存在为止。
```
...
​	不会，因为call改变的是返回的函数的this，但是执行的函数在返回的函数的内部，已经被指定了。
...
​	“ele.style.属性名称”和“ele.style['属性名称']”

最终渲染出来的

非ie	document.defaultView对象的getComputedStyle(ele，null/伪类)方法

ie		currentStyle的属性，且用法和style用法相同

```
var myDiv = document.getElementById("myDiv");
var finalStyle = myDiv.currentStyle ? myDiv.currentStyle : document.defaultView.getComputedStyle(myDiv, null);/*利用判断是否支持currentStyle（是否为ie）来通过不同方法获取style*/
...
```
https://blog.csdn.net/NumbSilver/article/details/116891971
https://blog.csdn.net/Y0W1as5eg37urFdS/article/details/113777492			八股文二号
```
...
继续沿_proto_寻找，直到找到Object原型对象

若还没有，返回undefined，不报错
...
JS中栈结构保存变量，其实不太准备，栈结构中保存的是**变量对象/执行上下文**
...
```
https://www.cnblogs.com/echolun/p/10903290.html	js new一个对象的过程，实现一个简单的new方法
```
...
```
https://juejin.cn/post/6844903761949753352	区别
```
...
<img src="images/image-20210919163801835.png" alt="image-20210919163801835" style="zoom:67%;" />
...
```
https://juejin.cn/post/6844903620190666759	递归和json的方式
```

```
JSON.sringify 和 JSON.parse 简单的方法
...
```
https://blog.csdn.net/NumbSilver/article/details/116748339	实现通讯的几种方法
```
...
```
https://segmentfault.com/a/1190000018605776	垃圾回收机制
https://javascript.info/garbage-collection	原文连接
https://www.jianshu.com/p/a8a04fd00c3c	几种垃圾回收算法。
```
...
```
https://blog.csdn.net/qq877507054/article/details/51395830	Javascript之BOM与DOM讲解
```
...
<img src="images/image-20210919203952782.png" alt="image-20210919203952782" style="zoom: 67%;" />
...
```
https://blog.csdn.net/weixin_42283981/article/details/82086199		数组的常见操作方法
```

**includes函数，函数的参数为查找的值，如果数组中有被查找的值，则函数输出“true”**
...
```
https://blog.csdn.net/alokka/article/details/78814674	常用META标签整理和SEO优化
```
...
```
https://www.cnblogs.com/zhaof/p/6100472.html	js词法分析
```
...
```
https://juejin.cn/post/6956170676327677966		通过面试题研究JavaScript数据类型转换
```
