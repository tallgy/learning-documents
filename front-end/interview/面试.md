...
```
 ...
 Position属性值  详细说了下sticky效果  absolute相对于谁 

把页面分成多少列，多少行的布局  没太搞懂面试官想问什么 

一个div里面有三个子div，让它们各占33%显示 

清除浮动的方法 

 Vue框架双向绑定，watcher实例有哪几种  get收集，set更新说反了 

 This指向 

 Let const var类型区别 说到了词法环境和变量环境 

浏览器输入url到渲染页面的整个过程  渲染过程详细说了下 提了一下html的解析过程，Render树和dom树并非一一对应，以及对于脱离文档流元素的处理

  

 Dom事件传递

给一个div同时监听捕获事件与冒泡事件，点击给div哪一个回调先执行  说错了，答案应该是对于目标元素而言，按定义的顺序执行，非目标元素先捕获再冒泡

  

 Git代码回滚  说了一下三种模式  提交到远程仓库后怎么撤回 

 Webpack如何提高打包速度

 手写 bind， apply ，[斐波那契数列]()

 面试总体体验还是很好的，问的问题也相对基础，在面试结束时就马上约二面了
